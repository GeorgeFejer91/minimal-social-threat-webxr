# Minimal Social Threat Top-down + WebXR

A deterministic social-threat research prototype with live top-down/3D browser views and optional WebXR with automatic HRTF spatial audio:
...
**Direct views:** [live top-down/3D preview](https://georgefejer91.github.io/minimal-social-threat-webxr/?view=trial) · [dedicated headset host](https://georgefejer91.github.io/minimal-social-threat-webxr/?view=headset) · [PC operator view](https://georgefejer91.github.io/minimal-social-threat-webxr/?view=companion)

- **Top-down / 3D browser preview:** the large main viewport switches between a clean planar state monitor and the complete Three.js scene while preserving the same authoritative scenario state. Twelve minimalist or human-form agents form six conversational dyads in a large crowd entirely in front of the observer. After a longer positive exchange, a fading shrouded shadow, visible angry agent, or articulated spider approaches continuously while individual awareness, fear, avoidance, escape distance, and motion build gradually.
...
The 3D scene supports a dusk forest-clearing or neutral study-grid background. The forest uses deterministic project-authored broadleaf and conifer geometry, layered undergrowth, depth haze, and a tested central corridor that keeps every tree crown outside the threat route. The optional add-on supports WebXR passthrough (`immersive-ar`). Procedural visuals and all sounds are generated locally; the WebXR view can additionally load the locally bundled Cesium Man GLB. The spider is a project-authored, viewer-facing articulated rig with an alternating eight-leg gait. The landing, trial, and companion views use a safe-area-aware single-column phone layout with touch-sized controls; both the planar Canvas and optional WebGL surfaces yield vertical gestures to page scrolling.
...
- facial-expression identity, intensity, transition naturalness, common category confusions, and recognition at actual 3D/XR crowd distance.

The threat has a code-tested **1.8 m minimum distance**. A 12-second positive baseline contains alternating calls, gentle replies, acknowledgements, and soft group-like murmurs. The threat then moves without stopping for 36 seconds in “Gentle” or 26 seconds in “Standard”; the first six seconds are labeled emerging awareness while approach and fade are already underway. Agent escape paths progressively widen and deepen, and gait amplitude follows actual translation so figures settle instead of running in place. **Start preview** starts the browser trial and complete cue schedule; immersive launch continues or starts the same trial and spatial-audio graph. Right-controller A restarts/resumes, while browser controls, either XR controller trigger, and the companion can pause. Reset/completion or leaving immersion closes its active graph. Digital limiting and the −18→0 dB relative ramp do not establish a safe sound-pressure level; begin at low volume and calibrate physical output before participant use.
...
| --- | --- | --- |
| Scenario clock, phase, social links, agent behavior/awareness/fear/avoidance/locomotion, audio cues, threat distance | Participant/headset host | Versioned scene frame at up to 20 Hz; immersive time is advanced from the active XR frame loop |
...
| Rendering | Each browser | Top-down planar or 3D reconstruction from the same latest accepted snapshot |
| Logging | Participant/headset host | Downloadable bounded CSV; never uploaded by this app |

The scene transport follows the Affect Tracker’s remote-Flubber pattern: bundled VDO.Ninja SDK 1.5.5, a data-only public discovery room, anonymous random stream IDs, no media tracks, `ordered: false`, `maxRetransmits: 0`, newest-state backpressure, sequence validation, automatic selection only when one source exists, and an explicit stale state. Transport envelope/command version 2 carries scenario schema 6 unchanged. The snapshot includes the complete twelve-agent social/gradual-motion state, avatar style, active audio-cue metadata, one of three threat kinds, and threat visibility rather than Affect Tracker’s 12-byte X/Y frame. The v2 namespace separates these peers from incompatible earlier clients.
...
- direct immersive launch/start, non-frozen XR-frame clock progression, browser-to-XR clock continuity, right-controller A-button restart/resume, and controller-trigger pause;
...
- `lib/facial-expression.ts` — FACS-informed SVG geometry, all-pairs/weighted morphing, SVG serialization, and spherical projection math.
...
- `components/TopdownScene.tsx` — shared main-screen and companion planar Canvas renderer.
