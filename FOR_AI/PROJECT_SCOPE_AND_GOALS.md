...
Build one deterministic social-threat scenario that works first as a touch-friendly 2D browser trial and optionally as virtual or mixed reality. Six minimal game-like social agents form three conversational dyads around the observer. A generic shrouded shadow with faint red eyes emerges, the agents detect it at different times, alarm spreads through the group, and they adopt varied avoidance behavior while the threat approaches but never crosses the configured 1.8 m boundary.
...
- Spatialize agent vocal cues and the approaching threat so sound direction and distance correspond to scene position.
...
- Six procedural agents, organized as three dyads.
...
- A generic shrouded shadow or an angry-agent comparison condition.
...
- Captioned, procedurally synthesized non-lexical social vocal cues.
