...
- Audio is scoped to an active immersive session and never starts without the trusted local participant/headset action that enters VR/MR. No persisted, operator-controlled, or participant-facing audio preference exists.
- `StudyApp.tsx` dispatches `requestSession()` first and starts Web Audio within the same unawaited trusted action. Once both are ready it feeds authoritative scene cues while `XrScene.tsx` updates the listener from the XR camera. Entry failure or session exit disposes the audio graph; the non-immersive page remains silent.
