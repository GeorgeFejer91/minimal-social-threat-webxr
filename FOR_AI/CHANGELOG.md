...
- Loaded public source `7346177` in Quest Browser 149 and confirmed WebXR VR/MR support, engine readiness, dedicated-host broadcasting, the new main-stage controls, and a clean JavaScript runtime. A PC companion selected that exact headset host and received confirmed Start/Pause/Reset receipts plus authoritative readback. Because the unworn headset returned to Home/sleep, this bounded run is not current-source immersive visual acceptance; the temporary diagnostic forward was removed and no device audio, proximity, or persistent power setting was changed.
- That Quest readback also caught a prewarm-transition defect: the 3D host expanded while its WebGL drawing buffer could remain at 2×2. `XrScene` now receives explicit visibility state and synchronously resizes the camera and renderer when the main 3D tab is selected. Source `f4c432a` / Pages deployment `416f6aa` was then rechecked on Quest Browser: the 3D buffer expanded to 831×518 inside the 832×518 stage with a live WebGL context and no runtime exception. This verifies the ordinary browser renderer transition, not an unworn immersive session.
