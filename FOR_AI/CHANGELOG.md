...
- That Quest readback also caught a prewarm-transition defect: the 3D host expanded while its WebGL drawing buffer could remain at 2×2. `XrScene` now receives explicit visibility state and synchronously resizes the camera and renderer when the main 3D tab is selected; a source gate protects this behavior pending public-device re-verification.
