...
| SVG facial-expression geometry | Neutral plus happiness, sadness, fear, anger, surprise, and disgust; pairwise/weighted cubic-Bézier morphing; 2D Canvas rendering and equirectangular sphere wrapping | Project-authored `lib/facial-expression.ts`, informed by FACS patterns and the facial-expression studies in the bibliography | MIT project code; no external face image or model is redistributed; exact graphics and transitions are unvalidated | Cite the motivating FACS/recognition/dynamics sources as design rationale only. Report target-population validation of the exact rendered stimuli before study use. |
